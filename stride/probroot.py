...
class BranchProbModel_corrected(object):
    ...
    def P_o_G_T(toA, A, B):
        ...
        """
        nA = 2.0 * len(A) - 2  # number of branches in A part of tree
        nB = 2.0 * len(B) - 2  # number of branches in B part of tree
...
class PoissonModel_IntergrateBranchLenthsSumFP(BranchProbModel_corrected):
    ...
    def __init__(
        self, rel_FP_rate_nonterm=0.001, rel_FP_rate_term=0.001, qExcludeTerminals=False
    ):
        """
    ...
    def Ps_o_G_d(self, A, B, countsA, countsB):
        ...
        *** Resolve Numerical Overflows: 3 Use logarithms ***
        ...
        lnx = elnprod(
            self.lnP_d_G_o(countsA, countsB, True, isTerminal),
            eln(self.P_o_G_T(True, A, B)),
        )  # P(d|->A, T)
        lny = elnprod(
            self.lnP_d_G_o(countsA, countsB, False, isTerminal),
            eln(self.P_o_G_T(False, A, B)),
        )  # P(d|<-A, T)
        lnz = elnprod(
            self.lnP_d_G_o(countsA, countsB, None, isTerminal),
            eln(self.P_o_G_T(None, A, B)),
        )  # P(d|r, T)
        lntot = elnsum(
            elnsum(lnx, lnz), lny
        )  # both x or y could be small but not x & z (or y & z)
...
class PoissonModel_WithTeminalModel(BranchProbModel_corrected):
    ...
    def Ps_o_G_d(self, A, B, countsA, countsB):
        if len(A) == 1 or len(B) == 1:
            ...
            if len(A) == 1:
                lnx = elnprod(
                    self.lnP_d_G_o_term(countsB, False), eln(self.P_o_G_T(True, A, B))
                )
                lny = None
                lnz = elnprod(
                    self.lnP_d_G_o_term(countsB, True), eln(self.P_o_G_T(None, A, B))
                )
            else:
                lnx = None
                lny = elnprod(
                    self.lnP_d_G_o_term(countsA, False), eln(self.P_o_G_T(False, A, B))
                )
                lnz = elnprod(
                    self.lnP_d_G_o_term(countsA, True), eln(self.P_o_G_T(None, A, B))
                )
        else:
            lnx = elnprod(
                self.lnP_d_G_o(countsA, countsB, True), eln(self.P_o_G_T(True, A, B))
            )  # P(d|->A, T)
            lny = elnprod(
                self.lnP_d_G_o(countsA, countsB, False), eln(self.P_o_G_T(False, A, B))
            )  # P(d|<-A, T)
            lnz = elnprod(
                self.lnP_d_G_o(countsA, countsB, None), eln(self.P_o_G_T(None, A, B))
            )  # P(d|r, T)
        lntot = elnsum(
            elnsum(lnx, lnz), lny
        )  # both x or y could be small but not x & z (or y & z)
...
def P_clade(node, probs, p_cond):
    ...
    if cluster in probs:
        ...
        l, r = node.children
        p = (
            P_clade(l, probs, p_cond)
            * Pc(l, p_cond)
            * P_clade(r, probs, p_cond)
            * Pc(r, p_cond)
        )
        probs[cluster] = p
        #        print((p, cluster))
...
def GetBranchProbs(supported_clusters_counter, biparts, probModel):
    ...
    for A, B in biparts:
        if len(A) == 0 or len(B) == 0:
            continue
        x, y, z = probModel.Ps_o_G_d(
            A, B, supported_clusters_counter[A], supported_clusters_counter[B]
        )
        p_cond[A] = (x, y, z)
        p_cond[B] = (y, x, z)
    return p_cond
...
def GetFinalProbs(biparts, p_cond, tree):
    ...
    for A, B in biparts:
        ...
        if n2 != tree:
            tree.set_outgroup(n2)
        l, r = tree.children
        p = (
            P_clade(l, probs_clades, p_cond)
            * Pc_root(l, p_cond)
            * P_clade(r, probs_clades, p_cond)
        )
        total += p
        p_final[A] = p
    #        print(p, A, 'final')
...
def GetTerminalRates(allSpecies, clades, supported_clusters_counter):
    ...
    termDups = sorted(
        [v for k, v in supported_clusters_counter.items() if len(k) == nAntiTerm],
        reverse=True,
    )
    nSp = float(len(allSpecies))
    ntp = sum(termDups[:2]) / 2.0
    nfp = sum(termDups)
    rtp = ntp + 1
    rfp = (nfp + 1) / nSp
...
def GetAlpha(allSpecies, clades, supported_clusters_counter):
    ...
    nTerminal = sum(
        [
            v
            for k, v in supported_clusters_counter.items()
            if (len(k) == 1 or len(k) == nSp - 1)
        ]
    )
    nNonTerminal = sum(
        [
    ...
    alpha_nonTerm = float(nFP_mean + 1.0) / float(nNonTerminal - nFP_mean + 1.0)
    nFP_mean = np.mean(nFP_term)
    alpha_term = float(np.mean(nFP_term) + 1.0) / float(nTerminal - nFP_mean)
