...
class Node(object):
    ...
    def get_up_grand_species_clades(self):
        ...
        ancestors = (
            self.node.get_ancestors()
        )  # ordered list starting with parent and progressing upwards
        c = ancestors[0]
        ...
        if c.is_root() and len(ch_temp) == 3:
            ...
            c1, c2 = (
                ch_temp[:2]
                if ch_temp[2] == self.node
                else ch_temp[1:3]
                if ch_temp[0] == self.node
                else (ch_temp[0], ch_temp[2])
            )
            ...
            if c2.is_root() and len(ch_temp) == 3:
                ...
                c22clade = (
                    c2.sp_up
                )  # c21clade = everything not in the others, work out at the end
            else:
                raise Exception("Expected binary tree")
    ...
    def get_up_genes(self, nMax):
        ...
        if nUp > nMax:
            ...
            return frozenset(self.node.get_tree_root().get_leaf_names()).difference(
                frozenset(self.node.get_leaf_names())
            )
    ...
    def get_gene_sets(self, i, j, nMax=2000):
        ...
        if self.node.is_root():
            ...
            return (
                tooLarge
                if len(children[i]) > nMax
                else frozenset(children[i].get_leaf_names()),
                tooLarge
                if len(children[j]) > nMax
                else frozenset(children[j].get_leaf_names()),
            )
        else:
            children = self.node.get_children()
            if len(children) != 2:
            ...
            if i <= 1:
                iGenes = (
                    frozenset(children[i].get_leaf_names())
                    if len(children[i]) < nMax
                    else tooLarge
                )
            else:
            ...
            if j <= 1:
                jGenes = (
                    frozenset(children[j].get_leaf_names())
                    if len(children[j]) < nMax
                    else tooLarge
                )
            else:
    ...
    def ToSpecies(clades_of_clades, GeneMap):
        return [
            [set(map(GeneMap, [gene for gene in grandchild])) for grandchild in child]
            for child in clades_of_clades
        ]
...
def StoreSpeciesSets(t, GeneMap, allTaxa):
    for node in t.traverse("postorder"):
        if node.is_leaf():
            ...
            node.add_feature(
                "sp_down", set.union(*[ch.sp_down for ch in node.get_children()])
            )
    for node in t.traverse("preorder"):
        if node.is_root():
            ...
            if parent.is_root():
                ...
                node.add_feature(
                    "sp_up", set.union(*[other.sp_down for other in others])
                )
            else:
                others = [ch for ch in parent.get_children() if ch != node]
...
def SaveTree(tree, root_clade, cladeName, treeName, iExample):
    fn = outputDir + "/%s_%s_%d_%d.tre" % (
        cladeName,
        os.path.split(treeName)[1].split(".")[0],
        iExample,
        len(root_clade),
    )
...
def StoreGeneSets(t):
    for node in t.traverse("postorder"):
        if node.is_leaf():
            ...
            node.add_feature(
                "g_down", [g for ch in node.get_children() for g in ch.g_down]
            )
    for node in t.traverse("preorder"):
        if node.is_root():
...
def LocalCheck_clades(clade1, clade2, expClades, GeneToSpecies):
    ...
    x0, x1 = (
        expClades[0]
        if len(expClades[0]) == 2
        else (None, None)
        if len(expClades[0]) == 1
        else (Exception, Exception)
    )
    y0, y1 = (
        expClades[1]
        if len(expClades[1]) == 2
        else (None, None)
        if len(expClades[1]) == 1
        else (Exception, Exception)
    )
    ...
    for actClades in [clade1, clade2]:
        ...
        for clade in actClades:
            if len(clade) == 1:
                ...
                c0 = clade[0]
                c1 = clade[1]
                if (iUsed != 0 and x0 != None) and (
                    (compare(x0, c0) and compare(x1, c1))
                    or (compare(x1, c0) and compare(x0, c1))
                ):
                    iUsed = 0
                    continue
                    ...
                    iUsed = 0
                    continue
                elif (iUsed != 1 and y0 != None) and (
                    (compare(y0, c0) and compare(y1, c1))
                    or (compare(y1, c0) and compare(y0, c1))
                ):
                    iUsed = 1
                    continue
    ...
    for counter, n in enumerate(t.traverse()):
        ...
        for i, j in itertools.combinations(range(3), 2):
            ...
            for kk in dict_clades:
                if s1.issubset(kk) and len(kk) < len(k1):
                    k1 = kk
                if s2.issubset(kk) and len(kk) < len(k2):
                    k2 = kk
            edges1 = set(
                [kk for kk in dict_clades if s1.issubset(kk) and len(kk) == len(k1)]
            )
            edges2 = set(
                [kk for kk in dict_clades if s2.issubset(kk) and len(kk) == len(k2)]
            )
            for k1 in edges1.intersection(edges2):
                if len(k1) == 1:
                    ...
                    [
                        not clade.isdisjoint(s1)
                        for clade0 in dict_clades[k1]
                        for clade in clade0
                    ]
                ) and all(
                    [
                        not clade.isdisjoint(s2)
                        for clade0 in dict_clades[k1]
                        for clade in clade0
                    ]
                ):
                    ...
                    if clades == None:
                        ...
                        if (
                            Join(clades[0]) != spSets[0]
                            or Join(clades[1]) != spSets[1]
                            or Join(clades[2]) != spSets[2]
                        ):
                            print(clades[0])
                            print(spSets[0])
                            print("")
                        ...
                        if clades == None:
                            break  # locally non-binary in vicinity of node, skip to next node
                    if not LocalCheck_clades(
                        clades[i], clades[j], dict_clades[k1], GeneToSpecies
                    ):
                        continue
                    ...
                    if qWriteDupTrees:
                        ...
                        SaveTree(
                            t_write, gSets[ii], clade_names[k1], treeName, iExample
                        )
                        iExample += 1
...
def AnalyseSpeciesTree(speciesTree):
    ...
    dict_clades = (
        dict()
    )  # dictionary of clades we require evidence of duplicates from for each partition
    clade_names = dict()
    for p in parts:
        if len(p) == nSpecies:
            continue
        ...
        dict_clades[p] = [
            [set(c.get_leaf_names()) for c in ch0],
            [set(c.get_leaf_names()) for c in ch1],
        ]
    ...
    for clade, score in contradictions.items():
        if score == m:
            if len(clade) > len(allSpecies) / 2:
                ...
                if allSpecies.difference(clade) not in roots:
                    roots.append(clade)
            else:
                roots.append(clade)
...
def GetRoot(
    ...
    ******* The Main method *******
    ...
    with open(gtree_path, "r") as fh:
        gtrees_newick = []
        for line in fh:
            gtrees_newick.append(line.strip())
    list_of_dicts = pool.map(
        SupportedHierachies_wrapper2,
        [
            (
                gtree_newick,
                GeneToSpeciesMap,
            ...
            for gtree_newick in gtrees_newick
        ],
    )
    clusters = Counter()
    ...
    if len(clusters) > 0 and len(roots) > 1:
        ...
        for r in roots:
            ...
            topDist.append(
                min(
                    [
                        speciesTree.get_distance(n, topology_only=True)
                        for n in speciesTree.traverse("preorder")
                        if frozenset(n.get_leaf_names()) in clusters
                    ]
                )
            )
            branchDist.append(
                min(
                    [
                        speciesTree.get_distance(n)
                        for n in speciesTree.traverse("preorder")
                        if (
                            frozenset(n.get_leaf_names()) in clusters
                            and speciesTree.get_distance(n, topology_only=True)
                            == topDist[-1]
                        )
                    ]
                )
    ...
    if qWriteRootedTree:
        for i, r in enumerate(roots):
            speciesTree = RootAtClade(speciesTree, r)
            speciesTree_rootedFN = (
                os.path.splitext(speciesTreeFN)[0] + "_%d_rooted.txt" % i
            )
            #    speciesTree = LabelNodes()
            speciesTree.write(
                outfile=speciesTree_rootedFN, format=2 if qHaveBranchSupport else 1
            )
...
def PrintRootingSummary(roots, clusters_counter, nSupport):
    ...
    if len(roots) > 1:
        print(
            "Identified %d non-terminal duplications.\n%d support the best roots and %d contradict them."
            % (n_non_trivial, n_non_trivial - nFP_mp, nFP_mp)
        )
    else:
        print(
            "Identified %d non-terminal duplications.\n%d support the best root and %d contradict it."
            % (n_non_trivial, n_non_trivial - nFP_mp, nFP_mp)
        )
...
def GetCluseterName(species_tree, S, cluster):
    ...
    if n == species_tree or (
        n.up == species_tree and len(S.difference(cluster)) < len(cluster)
    ):
...
def WriteResults(
    species_tree_fn_or_text, roots, S, clades, clusters_counter, output_dir, prefix=None
):
    ...
    for n in species_tree.traverse():
        if not n.is_leaf():
            ...
            iNode += 1
    s_tree_name = (
        output_dir + "Species_tree_labelled.tre" if prefix is None else prefix + ".tree"
    )
    ...
    if qBinary:
        ...
        print(
            "Probability distribution for root location is not supported for non-binary trees"
        )
        print(
            "To get a probability distribution for the root, please supply a fully resolved input species tree"
        )
    ...
    for clade in clades + [frozenset([s]) for s in S]:
        ...
        table[node.name] = [
            node.name,
            "X" if (clade in roots or anticlade in roots) else "",
            "%0.1f%%" % (100.0 * p),
            X,
            clusters_counter[y],
        ]
    csv_file_name = (
        output_dir + "Duplication_counts.csv" if prefix is None else prefix + ".csv"
    )
    with open(csv_file_name, "wb") as outfile:
        ...
        writer.writerow(
            [
                ...
                "MP Root",
                "Probability",
                "Duplications supporting clade",
                "Duplications supporting opposite clade",
            ]
        )
...
****************************************************************************"""
    ...
    if not args.directory:
        ...
        species, dict_clades, clade_names = AnalyseSpeciesTree(speciesTree)
        c, stride_dup_genes = SupportedHierachies_wrapper(
            args.gene_trees, GeneToSpecies, species, dict_clades, clade_names
        )
        for k, v in c.items():
            print((k, v))
    elif args.debug:
        ...
        for fn in glob.glob(args.gene_trees + "/*"):
            c, stride_dup_genes = SupportedHierachies_wrapper(
                fn, GeneToSpecies, species, dict_clades, clade_names
            )
        ...
        (
            ...
            species,
            all_stride_dup_genes,
        ) = GetRoot(args.Species_tree, args.gene_trees, GeneToSpecies, nProcs)
        PrintRootingSummary(roots, clusters_counter, nSupport)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "gene_trees",
        help="Directory conaining gene trees (with -d argument) or filename of a single gene tree to analyse (no -d argument)",
    )
    parser.add_argument(
        "-s",
        "--separator",
    ...
    parser.add_argument(
        "-S", "--Species_tree", help="Unrooted species tree in newick format"
    )
    parser.add_argument(
        "-d",
        "--directory",
        action="store_true",
        help="Process all trees in input directory",
    )
    parser.add_argument(
        "--debug", action="store_true", help="Run in serial to enable easier debugging"
    )
